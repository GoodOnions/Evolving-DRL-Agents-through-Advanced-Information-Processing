...
class ProcessFrame84(gym.ObservationWrapper):
    ...
    def process(frame):
        ...
        if frame.size == 240 * 256 * 3:
            ...
            elif input_type == 'shadows':
                img = segmentator.segment(img_og)
                state_img_stack.append(img)
                state_img_stack = state_img_stack[-4:]
                img = merge_frames(state_img_stack)
...
def show_state(env, ep=0, info=""):
    ...
    cv2.imshow("Processed Output!", img) #Display using opencv)
    cv2.waitKey(1)
